...
const WorkTimelineItem: React.FC<WorkTimelineItemProps> = ({ experience, index, setActive }) => {
  ...
  const companyLabel = str(locale, experience.company);
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 left-4 md:left-1/2 transform -translate-y-1/2 ${index % 2 === 0 ? "md:translate-x-8" : "md:-translate-x-8"} bg-primary text-primary-foreground rounded-full px-4 py-1`}
      >
        {experience.year}
      </div>
      ...
      >
        <div className="flex items-center mb-4">
          ...
          <div>
            <h3 className="text-xl font-semibold">{str(locale, experience.position)}</h3>
            <p className="text-primary">{companyLabel}</p>
            <p className="text-sm text-gray-500">{str(locale, experience.duration)}</p>
          </div>
        </div>
